...
var createSign = function (openid, scIp, callback) {
	
	var scIp = scIp.split(':')[scIp.split(':').length - 1];
	...
	var req = https.request(options, function (res) {
		...
		res.on('data', function (chunk) {
			...
			parser.parseString(bodyChunks, function (err, data) {
				callback(data);
			});
		});
